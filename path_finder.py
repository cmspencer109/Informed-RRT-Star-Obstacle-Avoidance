...
# Global constants
OBSTACLE_BUFFER: int = 0  # buffer obstacles for gps inaccuracies (meters)
...
def solve(
    ...
    for i in range(len(waypoints_points) - 1):
        ...
        print(f"No direct path, solving between waypoints {i} and {i+1}")
        ...
        if graph.success:
            ...
            if debug_plot:
                debug_path = [(p, -1) for p in path] # height data would normally be displayed, add -1 for debugging
                plotter.plot(
                    obstacles, boundary, graph=graph, path=debug_path, informed_boundary=informed_boundary
                )
    ...
    if debug_plot:
        print("Total runtime not available with debug plots enabled")
    else:
...
if __name__ == "__main__":
    ...
    # Alternative dataset
    # altitude, radius, and height should be in meters
    data_boundary: List[Dict[str, float]] = [
    ...
    data_waypoints: List[Dict[str, float]] = [
        {"latitude": 37.947286878699536, "longitude": -91.78228398250506, "altitude": 150.0},
        {"latitude": 37.95086689181623, "longitude": -91.78656411699848, "altitude": 75.0},
        {"latitude": 37.95086689181623, "longitude": -91.78656411699848, "altitude": 75.0},
    ]

    data_obstacles: List[Dict[str, float]] = [
        {"latitude": 37.94986244280119, "longitude": -91.78386816205061, "radius": 20*3.28084, "height": 400},
        {"latitude": 37.949388706571064, "longitude": -91.78569200437985, "radius": 40*3.28084, "height": 200},
        {"latitude": 37.94828884469052, "longitude": -91.78546673913256, "radius": 40*3.28084, "height": 150},
        {"latitude": 37.948861243306304, "longitude": -91.7830360828154, "radius": 30*3.28084, "height": 300},
        {"latitude": 37.947987798615436, "longitude": -91.7835146921038, "radius": 35*3.28084, "height": 300},
        {"latitude": 37.95007973521904, "longitude": -91.78522401099094, "radius": 50*3.28084, "height": 300},
    ]

    route = solve(data_boundary, data_obstacles, data_waypoints, show_plot=True, debug_plot=False)
